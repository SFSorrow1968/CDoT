...
- If you change or add a `ModOptionString` label/value, update any parsing in `CDoTModOptions` and any UI sync in `CDoTModOptionVisibility`.
...
- Always build Release + Nomad and copy outputs to `builds/CDoT-PCVR/CDoT/CDoT.dll` and `builds/CDoT-Nomad/CDoT/CDoT.dll`, then commit.
