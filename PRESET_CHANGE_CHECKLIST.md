...
- If you change or add a `ModOptionString` label/value, update any parsing in `BDOTModOptions` and any UI sync in `BDOTModOptionVisibility`.
...
- Always build Release + Nomad and copy outputs to `builds/BDOT-PCVR/BDOT/BDOT.dll` and `builds/BDOT-Nomad/BDOT/BDOT.dll`, then commit.
